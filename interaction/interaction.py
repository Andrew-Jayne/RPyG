    ...
    def at_merchant(player_party_instance: PlayerParty) -> None:
        ...
        match config.GLOBAL_GAME_MODE:
            ...
            case "MANUAL":
                ...
                for player_instance in player_party_instance.members:
                    merchant_messages = [f"{player_instance.name}", f"Gold: {player_instance.gold}", f"Potions: {player_instance.potions}", "","Choose an Action:"]
                    
                    while player_choice != "LEAVE":
                        player_choice = __class__.prompt_user(merchant_options, merchant_messages)
