...
def set_encounter_targets(
    current_event_targets: str, player_party_instance: PlayerParty
):
...
def execute_actor_action(
    ...
    for target in target_instance_list:
        match event_object["actor_action"]:
            ...
            case _:
                raise ValueError(
                    f"Error Invalid Method Call: {event_object['actor_action']}"
                )
...
def execute_special_action(event_object: dict, player_party_instance: PlayerParty):
    ...
    match event_object["special_action"]:
        ...
        case _:
            raise ValueError(
                f"error invalid special_action: {event_object['special_action']}"
            )
